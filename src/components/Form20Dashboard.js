...
const Form20Dashboard = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const momDataResponse = await axios.get(
          `http://15.206.128.21:5000/api/form20/get-report/${userId}`,
          {
            headers: {
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByPC = await axios.get(
            `http://15.206.128.21:5000/api/form20/get-report-by-pc/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByConstituency = await axios.get(
            `http://15.206.128.21:5000/api/form20/get-report-by-constituency/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        console.error("Error fetching form20 data by constituency:", error);
      }
    } else if (name === "zone") {
      try {
        const token = await localforage.getItem("token");
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByZone = await axios.get(
            `http://15.206.128.21:5000/api/form20/get-report-by-zone/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        console.error("Error fetching form20 data by zone:", error);
      }
    }
  };
  ...
  const handleDeleteMom = async (momId) => {
    try {
      const token = await localforage.getItem("token");
      await axios.delete(
        `http://15.206.128.21:5000/api/form20/delete-report/${momId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      window.alert("form20 is deleted successfully");
              ...
              <table className="mom-table">
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: "center" }}>Form 20</th>
                  </tr>
                </thead>
                <tbody>
...
export default Form20Dashboard;
