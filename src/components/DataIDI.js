...
const DataIDI = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const momDataResponse = await axios.get(
          `http://15.206.128.21:5000/api/idi/get-mom/${userId}`,
          {
            headers: {
      ...
      if (value === initialDropdownValue) {
        ...
        if (name === "pc") {
          const responseMomByPC = await axios.get(
            `http://15.206.128.21:5000/api/idi/get-mom-by-pc/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          const responseMomByConstituency = await axios.get(
            `http://15.206.128.21:5000/api/idi/get-mom-by-constituency/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          const responseMomByZone = await axios.get(
            `http://15.206.128.21:5000/api/idi/get-mom-by-zone/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const handleDeleteMom = async (momId) => {
    try {
      const token = await localforage.getItem("token");
      await axios.delete(`http://15.206.128.21:5000/api/idi/delete-mom/${momId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    try {
      ...
      if (selectedZone === initialDropdownValue) {
        ...
        const responseZoneCount = await axios.get(
          `http://15.206.128.21:5000/api/idi/get-mom-count-by-zone/${selectedZone}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    try {
      ...
      if (selectedPc === initialDropdownValue) {
        ...
        const responsePcCount = await axios.get(
          `http://15.206.128.21:5000/api/idi/get-mom-count-by-pc/${selectedPc}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    try {
      ...
      if (selectedConstituency === initialDropdownValue) {
        ...
        const responseAcCount = await axios.get(
          `http://15.206.128.21:5000/api/idi/get-mom-count-by-constituency/${selectedConstituency}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
            ...
            <div className="mom-count">
              ...
              >
                ...
                <div className="total-count" style={{ margin: "5px" }}>
                  <h3>Total IDI Count: </h3>
                  <h6>{responseAllMomCount}</h6>
                </div>
                <div className="total-count" style={{ margin: "5px" }}>
                  <h3>Zone-wise IDI Count: </h3>
                  <h6>{zoneCount}</h6>
                </div>
              </div>
              <div className="select-columns">
                <div className="total-count" style={{ margin: "5px" }}>
                  <h3>Pc-wise IDI Count: </h3>
                  <h6>{pcCount}</h6>
                </div>
                <div className="total-count" style={{ margin: "5px" }}>
                  <h3>Ac-wise IDI Count: </h3>
                  <h6>{acCount}</h6>
                </div>
              </div>

              <h2 className="party-header">IDI count by party</h2>
              ...
              <table className="mom-table">
                <thead>
                  <tr>
                    ...
                    >
                      Party Name{" "}
                      {getClassNamesFor("partyName") === "asc" && "↑"}
                    ...
                    <th
                      ...
                      className={getClassNamesFor("designation")}
                    >
                      Contact {getClassNamesFor("designation") === "asc" && "↑"}
                      {getClassNamesFor("designation") === "desc" && "↓"}
                    </th>
                    ...
                    <th style={{ textAlign: "center" }}>In-Depth Interviews</th>
                  </tr>
                </thead>
                <tbody>
                  {displayedMomData.map((mom) => (
                    <tr key={mom.id}>
                      ...
                      <td className="td-extra">
                        ...
                        >
                          {role !== "mod" &&
                          role !== "Northern Maharashtra" ? null : (
                            <button
                          ...
                          {mom.document && (
                            <a
                              href={mom.document}
                              download={`Document_of_${mom.respondentName}.pdf`}
