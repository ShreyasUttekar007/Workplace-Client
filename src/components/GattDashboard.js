...
const GattDashboard = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const momDataResponse = await axios.get(
          `http://15.206.128.21:5000/api/gatt/get-report/${userId}`,
          {
            headers: {
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByPC = await axios.get(
            `http://15.206.128.21:5000/api/gatt/get-report-by-pc/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByConstituency = await axios.get(
            `http://15.206.128.21:5000/api/gatt/get-report-by-constituency/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        console.error("Error fetching gatt data by constituency:", error);
      }
    } else if (name === "zone") {
      try {
        const token = await localforage.getItem("token");
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByZone = await axios.get(
            `http://15.206.128.21:5000/api/gatt/get-report-by-zone/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        console.error("Error fetching gatt data by zone:", error);
      }
    }
  };
  ...
  const handleDeleteMom = async (momId) => {
    try {
      const token = await localforage.getItem("token");
      await axios.delete(
        `http://15.206.128.21:5000/api/gatt/delete-report/${momId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      window.alert("gatt is deleted successfully");
              ...
              <table className="mom-table">
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: "center" }}>Gatt/Gann Report</th>
                  </tr>
                </thead>
                <tbody>
