...
const MomData = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const momDataResponse = await axios.get(
          `http://15.206.128.21:5000/api/moms/get-mom/${userId}`,
          {
            headers: {
      ...
      if (value === initialDropdownValue) {
        ...
        if (name === "pc") {
          const responseMomByPC = await axios.get(
            `http://15.206.128.21:5000/api/moms/get-mom-by-pc/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          const responseMomByConstituency = await axios.get(
            `http://15.206.128.21:5000/api/moms/get-mom-by-constituency/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          const responseMomByZone = await axios.get(
            `http://15.206.128.21:5000/api/moms/get-mom-by-zone/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const handleDeleteMom = async (momId) => {
    try {
      const token = await localforage.getItem("token");
      await axios.delete(`http://15.206.128.21:5000/api/moms/delete-mom/${momId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
